...
pf.prototype.render = function (k) {
  ...
  if (k) {
    this._anmition = requestAnimationFrame(
      function () {
        this.render(1)
      }.bind(this)
    )
...
pf.prototype.drawImg = function (param) {
  const img_w = this._img.width * param._scale
  const img_h = this._img.height * param._scale
